...
class TestQuizResult:
    ...
    def test_correct_result(self):
        ...
        card = Flashcard(term="DNS", definition="Domain Name System")
        result = QuizResult(
            flashcard=card, user_answer="Domain Name System", is_correct=True
        )
    ...
    def test_incorrect_result(self):
        ...
        result = QuizResult(
            flashcard=card, user_answer="Wrong Answer", is_correct=False
        )
...
class TestFlashcardLoader:
    ...
    def test_missing_flashcards_key(self):
        ...
        try:
            with pytest.raises(
                ValueError, match="must contain 'cards' or 'flashcards'"
            ):
                FlashcardLoader.load_from_json(temp_path)
        finally:
            Path(temp_path).unlink()
    ...
    def test_flashcard_not_object(self):
        ...
        try:
            with pytest.raises(
                ValueError, match="Flashcard at index 0 must be an object"
            ):
                FlashcardLoader.load_from_json(temp_path)
        finally:
            Path(temp_path).unlink()
